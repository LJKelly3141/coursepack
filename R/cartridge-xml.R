...
# ---- the iframe wrapper, defined once ------------------------------------
#
# THE ONE COPY OF THIS TEMPLATE. `extract_manifest()` classifies an exported
# page as a generated wrapper only when its body is exactly what this function
# returns and nothing else (review 9), so the writer and the reader have to be
# looking at the same template. A second copy would drift from this one and the
# extractor would start carrying pages this builder wrote.
IFRAME_ALLOW <- paste0(' allow="accelerometer; clipboard-write; encrypted-media;',
                       ' gyroscope; picture-in-picture"')

iframe_wrapper <- function(ftitle, src, width, height, allow = "") {
  paste0(
'<div style="width: 100%; max-width: 100%; margin: 0 auto;">\n',
'  <iframe title="', xesc(ftitle), '" src="', xesc(src), '" width="', width,
'" height="', height, '" style="border: 0;" loading="lazy" allowfullscreen=""',
allow, '></iframe>\n',
...
for (k in names(pages)) {
  ...
  if (isTRUE(p$body)) {
    ...
    allow <- if (isTRUE(p$video)) IFRAME_ALLOW else ''
    ...
    inner <- iframe_wrapper(ftitle, src, p$width, p$height, allow)
  }
